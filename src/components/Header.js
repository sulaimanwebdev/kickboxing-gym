...
const Header = (props) => {
  ...
  return (
    ...
    <div className="flex items-center justify-between gap-5 w-full bg-[#141414] px-5 h-[70px]">
      ...
      <div className="flex items-center gap-20 h-full">
        <Link to="/"><img src="/images/logo-white.svg" alt="logo" /></Link>
        {
           ...
           null
        :
        <div className={`flex transition 2md:translate-x-0 2md:items-center gap-3 fixed top-0 left-0 w-1/2 bg-[#141414] z-10 h-screen 2md:h-full flex-col 2md:flex-row 2md:bg-transparent 2md:w-auto 2md:relative 2md:left-auto 2md:top-auto ${leftbar}`}>
    ...
    <div onClick={()=>{setleftbar("-translate-x-full")}} className={`fixed top-0 left-0 w-full min-h-screen bg-black bg-opacity-50 ${leftbar === "-translate-x-full" ? "hidden" : "flex"}`}></div>
    </>
  )
}
